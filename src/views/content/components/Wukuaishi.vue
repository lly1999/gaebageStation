<template>
    <el-container>
        <el-header style="font-size:25px;padding:5px ;">
            <h5 class="card-title" style="font-size:30px;padding:5px ;">五块石垃圾站数据统计</h5>
        </el-header>
        <el-main>
...
onBeforeMount(() => {
    ...
    getQuery('五块石', 'transporter', today, tomorrow, 1, 10000).then(function (resp) {
        total.value = 0;
        for (let i = 0; i < resp.length; i++) {
            total.value = resp[i].netWeight + total.value
        }
        ...
        for (let i = 1; i < 7; i++) {
            ...
            getQuery('五块石', 'transporter', cal_day, cal_time, 1, 10000).then(function (resp) {
    ...
    getQuery('五块石', 'transporter', today, tomorrow, 1, 10).then(function (resp) {
