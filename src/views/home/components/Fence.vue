<template>
              ...
              <el-table :data="tableItem.data" class="table" :fit="false" max-height="300px">
                ...
                <el-table-column label="操作" width="200">
                  <template #default="scope">
                    <el-button link size="mini" type="primary" @click="showFence(scope.$index)">查看</el-button>

                    <el-button size="mini" type="primary" @click.prevent="inEditFence(scope.$index)" plain text>编辑
                    </el-button>
                    <el-button size="mini" type="danger" @click="deleteFence(scope.$index,scope.row)" text>删除
                    </el-button>
                  </template>
                </el-table-column>
  ...
  <el-amap :center="polygon.center" :zoom="polygon.zoom" strokeStyle='dashed' :doubleClickZoom="polygon.doubleClickZoom"
    @init="init">
    <el-amap-control-geolocation :zoomToAccuracy="false" @complete="getLocation" position="LT" />
    <!-- 初始定位点 -->

    <!-- 嵌套信息窗体 -->
    <el-amap-info-window v-model:visible="visible" :position="polyPosition" anchor="top-right" direction="vertical">
    ...
    <!-- 多边形绘制 -->
    <el-amap-polygon :draggable="polygon.draggable" :path="polygon.path" :editable="polygon.edit" fillColor="ccebc5"
      strokeColor="#DC143C" bubble="true">
    ...
    <!-- <el-amap-polygon
        ...
        strokeColor="#DC143C"
      ></el-amap-polygon>  -->
    <el-amap-info-window v-for="(window,index) in windowList" :position="window.position"
      v-model:visible="window.visible" anchor="top-left">
      <div>
        <el-descriptions title="人员信息" border direction="vertical">
          <el-descriptions-item label="姓名">{{window.personName }}</el-descriptions-item>
          <el-descriptions-item label="电话">
            <el-button plain type="primary" link>
              <el-icon :size="25" style="vertical-align: middle">
                <PhoneFilled />
              </el-icon>
            </el-button>{{window.tel}}
          </el-descriptions-item>
          <el-descriptions-item label="微信">
            <el-button plain type="primary" link>
              <el-image class="logo-icon" :src="require('@/assets/home/icon32_wx_logo.png')" :size="25"></el-image>
            </el-button>{{window.vx}}
          </el-descriptions-item>
    ...
    <el-amap-info-window v-for="(window,index) in windowFenceList" :position="window.position"
      v-model:visible="window.visible" anchor="top-left">
      <div>
        <el-descriptions title="围栏信息" border direction="vertical">
  ...
  <el-dialog v-model="dialogFormVisible" title="请输入围栏信息" close-on-click-modal="false" close-on-press-escape="false">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmAddFence()">确认</el-button>
      </span>
    </template>
  </el-dialog>
  ...
  <el-dialog v-model="dialogEditFormVisible" title="编辑围栏信息" close-on-click-modal="false" close-on-press-escape="false">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmEditFence()">确认</el-button>
      </span>
    </template>
  </el-dialog>
